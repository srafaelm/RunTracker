...
/// </summary>
public record GetFullExportQuery(string UserId) : IRequest<byte[]>;
...
public class GetFullExportQueryHandler : IRequestHandler<GetFullExportQuery, byte[]>
{
    ...
    public async Task<byte[]> Handle(GetFullExportQuery request, CancellationToken ct)
    {
        ...
        using (var zip = new ZipArchive(ms, ZipArchiveMode.Create, leaveOpen: true))
        {
            ...
            foreach (var activity in activities)
            {
                ...
                var gpx = BuildGpx(activity, streams);
    ...
    private static string BuildCsv(List<RunTracker.Domain.Entities.Activity> activities)
    {
        var sb = new StringBuilder();
        sb.AppendLine("id,name,sport_type,start_date,distance_km,moving_time_s,elevation_m,avg_hr,avg_pace_min_km");
    ...
    private static string BuildJson(List<RunTracker.Domain.Entities.Activity> activities)
    {
        var sb = new StringBuilder("[");
        ...
        List<RunTracker.Domain.Entities.ActivityStream> streams)
    {
        ...
        sb.AppendLine($"  <trk><name>{System.Security.SecurityElement.Escape(activity.Name)}</name><trkseg>");
